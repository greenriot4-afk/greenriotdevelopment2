-- Agregar la comisión EUR convertida a USD (4.75 EUR ≈ 5.00 USD aproximadamente)
-- Primero verificar si ya se agregó esta comisión específica
DO $$
DECLARE
    wallet_id_var UUID;
    current_balance_var NUMERIC;
    commission_usd NUMERIC := 5.00; -- 4.75 EUR convertido a USD
BEGIN
    -- Buscar la wallet USD del usuario
    SELECT id, balance INTO wallet_id_var, current_balance_var
    FROM wallets 
